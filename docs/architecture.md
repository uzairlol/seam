...
For each agent and round, `BaseAgent.format_prompt()` creates a plain-text prompt. It includes the system prompt, agent ID, optional memory, the Python string representation of the observation dictionary, an available-actions block, and an instruction. The available-actions block has two renderings: when the action space spans more than twenty entries and begins with integer strings (as in number guessing), it is abbreviated to `Integers from <first> to <last> (<len> choices)`; otherwise up to the first twenty action-space entries are listed, with an ellipsis when the space is longer. Bargaining observations receive role-specific instructions: proposers are asked for two integers, while responders are asked for `accept` or `reject`; agents with any other role receive the generic instruction to respond with one of the valid action strings.
